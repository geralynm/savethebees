...
//parse wave header
 ...
 void th_handle(FILE *fp, struct hivedata *hive) {
	 if (!fp) {
	...
	fprintf(fp, " %d ", hive->humidity_flag);
...
void FFT_handle(FILE *fp, struct wave_header hdr, float* master_fft_array) {
...
int audio_compare(float *fft_array, int numsamples, struct wave_header *hdr, struct hivedata *hive) {
 ...
 int main(int argc, char** argv) {
   ...
   th_handle(hivefp, &hive);
   audio_compare(fft_array, sizeof(fft_array)/4, &hdr, &hive);
